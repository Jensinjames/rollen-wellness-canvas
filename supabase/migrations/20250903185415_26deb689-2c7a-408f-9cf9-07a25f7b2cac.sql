-- 2. Create database views for analytics and insights
...
SELECT 
  ...
  CASE 
    WHEN c.daily_time_goal_minutes > 0 THEN 
      ROUND((SUM(a.duration_minutes)::float / c.daily_time_goal_minutes) * 100, 2)
    ELSE 0
  END as daily_progress_percentage,
  CASE 
    WHEN c.weekly_time_goal_minutes > 0 THEN 
      ROUND((SUM(a.duration_minutes)::float / c.weekly_time_goal_minutes) * 100, 2)
    ELSE 0
