-- Fix security definer view issues by recreating with security_invoker

-- Drop existing views
DROP VIEW IF EXISTS public.category_totals;
DROP VIEW IF EXISTS public.subcategory_totals;
DROP VIEW IF EXISTS public.daily_streaks;
DROP VIEW IF EXISTS public.goal_deficiencies;
...
CREATE VIEW public.subcategory_totals 
WITH (security_invoker = true) AS
SELECT 
  sc.user_id,
...
WITH daily_activity AS (
  ...
  GROUP BY a.user_id, a.category_id, DATE(a.date_time)
),
streak_groups AS (
  SELECT 
    da.user_id,
    ...
    da.activity_date,
    da.total_minutes,
    da.activity_date - (ROW_NUMBER() OVER (PARTITION BY da.user_id, da.category_id ORDER BY da.activity_date))::int * INTERVAL '1 day' as streak_group
...
SELECT 
  ...
  c.name as category_name,
  c.color,
  MIN(sg.activity_date) as streak_start,
  MAX(sg.activity_date) as streak_end,
  COUNT(*) as streak_length,
  SUM(sg.total_minutes) as total_streak_minutes
FROM streak_groups sg
JOIN public.categories c ON sg.category_id = c.id AND sg.user_id = c.user_id
GROUP BY sg.user_id, sg.category_id, c.name, c.color, sg.streak_group
