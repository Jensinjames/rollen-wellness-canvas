 ...
 * Consolidates all validation logic with consistent interfaces
 */
...
export class ValidationService {
  ...
  static validateActivity(
    ...
    const durationValidation = this.validateDuration(data.duration_minutes, goalType);
  ...
  /**
   * Validate duration based on goal type
   */
  private static validateDuration(duration: number, goalType: string): NumberValidationResult {
    if (goalType === 'boolean') {
   ...
   * Sanitize form data
   */
  static sanitizeActivityData(data: ActivityFormData): ActivityFormData {
    return {
      ...data,
  ...
  static sanitizeCategoryData(data: CategoryFormData): CategoryFormData {
    return {
      ...data,
      name: data.name?.trim() || '',
